...
FULFILMENT = ((0, "Cancelled"), (1, "Self"), (2, "Zit"))
...
class VendorOrderItem(models.Model):
    order = models.ForeignKey(OrderLineItem, null=False, blank=False, on_delete=models.CASCADE, related_name='vendor_order_item')
    accept = models.IntegerField(choices=ACCEPT, default=0)
    fulfilment = models.IntegerField(choices=FULFILMENT, default=2)
    status = models.IntegerField(choices=STATUS, default=0)
    reason = models.IntegerField(choices=STATUS, null=True, blank=True)
    
    def __str__(self):
        return f"{self.order.product.title} in {self.order.order_number}"

    def save(self, *args, **kwargs):
        # Check if accept is set to "No"
        if self.accept == 2:  # Assuming 2 corresponds to "No"
            # Set fulfilment to "Cancelled"
            self.fulfillment = 0  # Assuming 0 corresponds to "Cancelled"
            self.status = 4
