...
def vendor_orders(request):
    ...
    orders = OrderLineItem.objects.filter(
        product__vendor=request.user).exclude(
        status=0
    )
...
def new_vendor_orders(request):
    ...
    orders = VendorOrder.objects.filter(
        item__product__vendor=request.user, accept=0
    ).order_by("-id")
    if orders.count() == 0:
        messages.info(
            request,
            f"There are no New orders."
...
def vendor_order_view(request, order_no, orderline_id):
    order_line_item = get_object_or_404(
        OrderLineItem, order__order_number=order_no, id=orderline_id
    )
    vendor_order, created = VendorOrder.objects.get_or_create(
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if vendor_order.reason > 0:
                ...
                order_line_item.save()
            messages.success(
                request, "Order status has been updated successfully!")
            return redirect("vendor_orders")
    ...
    history_entries = VendorOrderStatusHistory.objects.filter(
...
def accept_order(request, order_number, product_id):
    ...
    if vendor_order_item.accept == 0:
        ...
        messages.error(
            request,
            "Failed to accept order." "Kindly refresh the page.")
...
def shipment_type(request, order_number, product_id):
    ...
    if vendor_order_item.fulfilment == 2:
        ...
        vendor_order_item.save()
        product_status = 1
        message = "Shipment type has been set to self."
        " You still have to accept the order in other"
        " to begin shipment process."

    elif vendor_order_item.fulfilment == 1:
        vendor_order_item.fulfilment = 2
        vendor_order_item.save()
        product_status = 2
        message = "Shipment type has been set to Zit-Ship."
        " Kindly accept the order and prepare the order"
        " for pickup from Zit Ship Team."

    elif vendor_order_item.fulfilment == 0:
        message = "This Order has since been cancelled."
        " You can no longer ship. Contact Support for help."
...
def cancelled_vendor_orders(request):
    ...
    orders = OrderLineItem.objects.filter(
        product__vendor=request.user, status=5
    ).order_by("-id")
...
def cancel_order(request, order_number, product_id):
    ...
    order_line_item = get_object_or_404(
        OrderLineItem, order__order_number=order_number, product__id=product_id
    )
    if vendor_order_item.accept == 0:
        messages.error(
            request,
            "You are yet to accept/reject this shipment."
    ...
    if vendor_order_item.status == 4:
        ...
        return redirect(
            ...
            orderline_id=order_line_item.id
        )
...
def ship_order(request, order_number, product_id):
    ...
    if vendor_order_item.accept == 0:
        messages.error(
            request,
            "You are yet to accept this shipment."
            " Go to the new order sction and accept it first!",
        )
    else:
...
def delivery_failed_order(request, order_number, product_id):
    ...
    if vendor_order_item.status == 3:
        messages.success(
            request,
            "Your have set the order to Delivery failed!")
        return redirect(
            "vendor_order_view",
            order_no=order_number,
            orderline_id=order_line_item.id
...
def delivered_vendor_orders(request):
    ...
    orders = OrderLineItem.objects.filter(
        product__vendor=request.user, status=3
    ).order_by("-id")
...
def delivery_failed_vendor_orders(request):
    ...
    orders = OrderLineItem.objects.filter(
        product__vendor=request.user, status=4
    ).order_by("-id")
...
def print_order_receipt(request, order_no, orderline_id):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="order_{order_no}_receipt.pdf"'
    )
    ...
    if pisa_status.err:
        ...
        messages.success(
            ...
            " Find the downloaded pdf attachement on your device.",
        )
